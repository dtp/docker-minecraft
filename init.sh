...
case "$1" in
  "paperclip")
    ...
    wget --show-progress -qO /server.jar "https://ci.destroystokyo.com/job/PaperSpigot/lastSuccessfulBuild/artifact/paperclip.jar"
    ;;
  *)
    printf "Downloading version manifest...\n"
    wget --show-progress -qP .. "https://launchermeta.mojang.com/mc/game/version_manifest.json"
    wget --show-progress -qO /manifest.json \
    ...
    wget --show-progress -qO /server.jar "$(jq -r '.downloads.server.url' < /manifest.json)"
